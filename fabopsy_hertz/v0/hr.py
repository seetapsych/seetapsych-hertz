...
def compute_heart_beat(
    ...
    interp_times = _generate_interp_times(
        timestamps[0],
        timestamps[-1],
        step,
    )
    ...
    for i in range(3):
        interp_signal[i] = _gaussian_blur_1d(
            interp_signal[i]
        )

    pulse_signal = _chrominance_method(
        interp_signal,
    )

    # Smooth pulse signal.
    pulse_signal = _gaussian_blur_1d(
        pulse_signal
    )
    ...
    pulse_signal *= numpy.hamming(
        length
    )

    pulse_signal -= numpy.mean(
        pulse_signal
    )

    fft_result = numpy.fft.rfft(
        pulse_signal
    )
