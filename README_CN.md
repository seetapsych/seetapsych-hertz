...
TinyHR 从普通人脸视频的相邻帧中提取随血容量脉动产生的细微肤色变化，重建 rPPG 波形，
并通过频谱分析估计心率。
...
TinyHR 强调相邻人脸帧间与脉搏相关的肤色差异，预测与输入帧对齐的 rPPG 波形，
再通过确定性的带通滤波和 Welch 频谱分析得到心率。
...
| ⚡ | **低延迟推理** | 50 次预热后 CPU 测试的平均处理延迟为 132 ms；完成初始观测后，心率估计以 1.0 秒间隔更新。 |
...
| TinyHR 处理延迟 | **132 ms（均值）** | 输入归一化、ONNX 推理、波形归一化及心率信号处理 |
| 延迟分布 | **125 ms（中位数）；164 ms（P95）** | ONNX Runtime CPUExecutionProvider 预热后运行 50 次 |
| 首次观测窗口 | **30 FPS 时约 5.3 秒** | 首次估计前需要收集 160 帧 |
| 滚动更新间隔 | **默认 1.0 秒** | 30 FPS 时每 30 帧请求一次新估计 |

**延迟测试方法。** 使用 ONNX Runtime 1.30.0 CPUExecutionProvider，在预热后重复运行
50 次。所报告的计算时间不包含视频采集与人脸检测。

约 5.3 秒的初始时段对应 160 帧生理信号观测，132 ms 则表示后续模型推理与信号处理延迟。
该延迟低于 1.0 秒的更新间隔，可支持近实时健康交互、人机交互、情感计算研究及轻量级
远程监测原型。
